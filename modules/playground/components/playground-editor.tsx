...
interface PlaygroundEditorProps{
    ...
    onTriggerSuggestion:(editor:any,type:string) => void

}
    

const PlaygroundEditor = ({activeFile,content,onContentChange ,suggestion,onAcceptSuggestion,suggestionLoading,suggestionPosition,onRejectSuggestion,onTriggerSuggestion}:PlaygroundEditorProps) => {
    const editorRef = useRef<any>(null)
    const monacoRef = useRef<Monaco | null>(null)

    //adding other monaco related ref inorder to managae ai suggestions
    const inlineCompletionProviderRef = useRef<any>(null)
    const currentSuggestionRef = useRef<{text:string,position:{line:number,column:number},id:string}|null>(null)
    const suggestionTimeoutRef = useRef<NodeJS.Timeout | null>(null) //timer to clear the suggestion after some time if not accepted
    const tabCommandRef = useRef<any>(null) //to store the tab command disposable
    const isAcceptingSuggestionRef = useRef(false) // to prevent multiple acceptances
    const suggestionAcceptedRef = useRef(false) //to track if the suggestion was accepted
