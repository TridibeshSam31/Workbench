...
export const AIChatSidePanel: React.FC<AIChatSidePanelProps> = ({
  ...
  //this will be only on the frontend side no backend logic for exporting the chat 
  //rowser ke andar hi ek JSON file banata hai aur user ke system me download kara deta hai
  /*
  Blob = Binary Large Object
  Blob ek raw data container hota hai
jisme tum text, JSON, image, video, PDF — kuch bhi rakh sakte ho.
 


 Browser Blob ko bolta hai:
“Bas data rakh, baaki format/type mai samjha lunga”


Yeh basically:
📄 ek virtual file bana raha hai (RAM me)

Step 1: Chat data ready karna
const chatData = {
  messages,
...
JSON.stringify(...) → object ko text banaya
Blob(...) → text ko file jaisa treat karne laga
type → browser ko bata raha hai:

“Bhai yeh JSON file hai”
📌 Is moment par ek virtual .json file RAM me exist karti hai

Step 3: Blob → Downloadable URL
const url = URL.createObjectURL(blob);
eh line MOST CONFUSING hoti hai

Iska matlab:

Browser ke andar ek temporary URL bana diya jo is Blob ko point karta hai

Example:
blob:http://localhost:3000/3f1c9a...
❌ Yeh internet URL nahi
❌ Yeh server pe upload nahi
✅ Yeh sirf browser ke liye hai



Step 4: Fake <a> tag se download trigger
const a = document.createElement("a");
a.href = url;
a.download = "ai-chat-2026-01-04.json";
a.click();

Browser ka rule:

Agar <a> me download attribute ho
toh click = file download

👉 Tum manually click nahi kar rahe
👉 JS se click karwa rahe ho (automation)
  
  
Step 5: Cleanup (important but log ignore kar dete)
URL.revokeObjectURL(url);



asia hi similar flow cloudinary use krta hai ek bar uska flow dekhte hai 
File → Blob → FormData → Upload → Cloud → URL


but hmara code local downloads ke liye hai 
Data → Blob → Local URL → Download
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center gap-2">
                  <div className="hidden sm:flex items-center gap-2 text-xs text-zinc-400">
                    ...
                    >
                      <option value="gpt-6">gpt-5</option>
